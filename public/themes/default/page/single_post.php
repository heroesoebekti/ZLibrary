    ...
    <header class="mb-12 flex flex-col md:flex-row gap-6 md:gap-10 items-start">
        <div class="w-32 h-32 md:w-48 md:h-48 shrink-0 mx-auto md:mx-0">
            <div class="relative group h-full">
                <div class="absolute -inset-1 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-700"></div>
        ...
        <div class="flex-1">
            <div class="inline-block px-3 py-1 rounded-md bg-blue-50 text-blue-600 font-bold uppercase tracking-[0.2em] text-[9px] mb-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <?php foreach ($related_posts as $related): ?>
                <a href="<?= BASE_URL ?>/post/<?= $related['slug'] ?>" class="group flex items-center gap-4 p-3 rounded-2xl hover:bg-slate-50 transition border border-transparent hover:border-slate-100">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-slate-800 line-clamp-2 group-hover:text-blue-600 transition"><?= htmlspecialchars_decode($related['judul']) ?></h3>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                if (!empty($post['tags'])) {
                    ...
                    foreach ($tagsArray as $tag) {
                        $tag = trim($tag);
                        if (!empty($tag)) {
                            echo '<span class="text-[10px] bg-slate-100 px-3 py-1 rounded-full text-slate-600 hover:bg-blue-600 hover:text-white transition cursor-pointer">#' . htmlspecialchars_decode($tag) . '</span>';
                        }
                    }
                } else {
