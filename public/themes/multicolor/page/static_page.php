...
<nav class="container mx-auto px-6 pt-10">
    <ol class="flex items-center space-x-2 text-sm text-slate-400 font-medium dynamic-breadcrumb">
...
<article class="container mx-auto px-6 py-12 max-w-4xl">
    <header class="mb-16 pb-12 border-b border-slate-100">
        <div class="inline-block px-3 py-1 rounded-md font-bold uppercase tracking-[0.2em] text-[10px] mb-6 dynamic-post-badge">
        ...
        <div class="flex items-center gap-4 text-slate-500">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full flex items-center justify-center shadow-lg dynamic-post-author-icon">
    ...
    <div class="prose prose-slate prose-lg max-w-none text-slate-700 leading-relaxed 
                ...
                prose-p:mb-6 prose-blockquote:italic
    ...
    <div class="mt-24">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            <?php foreach ($related_posts as $related): ?>
                <a href="<?= BASE_URL ?>/post/<?= $related['slug'] ?>" class="group block border-l-2 border-transparent transition-all dynamic-related-link pl-4">
                    <h3 class="text-base font-bold text-slate-800 line-clamp-2 transition tracking-tight dynamic-related-title">
                        <?= htmlspecialchars_decode($related['judul']) ?>
                    </h3>
                    <span class="text-[10px] font-bold uppercase tracking-widest mt-2 block opacity-0 group-hover:opacity-100 transition dynamic-related-more"><?= __('Read More &rarr;') ?></span>
                </a>
            <?php endforeach; ?>
        </div>
    </div>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                if (!empty($post['tags'])) {
                    ...
                    foreach ($tagsArray as $tag) {
                        $tag = trim($tag);
                        if (!empty($tag)) {
                            echo '<span class="text-[11px] font-bold text-slate-500 transition cursor-pointer dynamic-tag-link">#' . htmlspecialchars($tag) . '</span>';
                        }
                    }
                }
                ?>
