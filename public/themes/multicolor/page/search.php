...
<section class="relative py-16 overflow-hidden dynamic-search-header">
    <div class="absolute top-0 right-0 -translate-y-12 translate-x-12 w-64 h-64 rounded-full blur-3xl dynamic-search-accent-blur"></div>
    ...
    <div class="container mx-auto px-6 relative z-10">
        ...
        <h1 class="text-3xl md:text-4xl font-extrabold text-white">
            <?= __('Search Results:') ?> <span class="dynamic-search-keyword">"<?= strip_tags($keyword) ?>"</span>
        </h1>
        <p class="text-slate-400 mt-2 flex items-center gap-2">
            <svg class="w-5 h-5 dynamic-search-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
    ...
    <div class="max-w-4xl mx-auto">
        <?php if (!empty($results) && is_array($results)): ?>
            <div class="grid gap-6">
                <?php foreach($results as $row): ?>
                    <article class="group bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 dynamic-search-card">
                        ...
                        <h2 class="text-2xl font-bold text-slate-900 transition dynamic-search-title">
                        ...
                        <div class="mt-5 pt-4 border-t border-slate-50 flex items-center justify-between">
                            <a href="<?= BASE_URL ?>/<?= $row['tipe'] ?>/<?= $row['slug'] ?>" class="inline-flex items-center gap-2 text-sm font-bold group/link dynamic-search-link">
                                <?= __('Read More') ?>
        ...
        <?php else: ?>
            <div class="text-center py-20 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                ...
                <div class="mt-8">
                    <a href="<?= BASE_URL ?>" class="px-8 py-3 text-white font-bold rounded-xl shadow-lg transition dynamic-search-btn">
                        <?= __('Back to Home') ?>
                    </a>
                </div>
            </div>
