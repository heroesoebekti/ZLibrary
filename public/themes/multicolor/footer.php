        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-8">
            <div class="flex flex-col md:flex-row items-center gap-2 md:gap-4 text-[11px] font-medium">
                ...
                <span class="text-slate-500 italic">
                    <?= __('Powered by') ?> 
                    <span class="btn-legal-trigger text-blue-500/80 font-bold not-italic cursor-pointer hover:text-blue-400" data-target="tagline"> 
                        ZLibrary CMS
                    </span>
                </span>
            </div>
            ...
            <div class="flex flex-wrap justify-center items-center gap-x-6 gap-y-3">
                <button class="btn-legal-trigger text-[10px] font-bold text-slate-500 hover:text-yellow-500 uppercase tracking-widest transition-colors" data-target="privasi"><?= __('Privacy') ?></button>
                <button class="btn-legal-trigger text-[10px] font-bold text-slate-500 hover:text-yellow-500 uppercase tracking-widest transition-colors" data-target="ketentuan"><?= __('Terms') ?></button>
                <button class="btn-legal-trigger text-[10px] font-bold text-slate-500 hover:text-yellow-500 uppercase tracking-widest transition-colors" data-target="bantuan"><?= __('Help') ?></button>
            </div>
        </div>
    </div>
</footer>
...
<div id="modal-legal" class="fixed inset-0 hidden items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4 transition-all duration-300" style="z-index: 99999 !important;">
    <div id="modal-content" class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden transform transition-all duration-300 scale-95 opacity-0">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            ...
            <button class="btn-close-modal text-slate-400 hover:text-slate-600 transition-colors">
        ...
        <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end">
            <button class="btn-close-modal px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-blue-500/20">
                <?= __('Close') ?>
            </button>
        </div>
    </div>
...
// Fungsi Utility Global (Didefinisikan di dalam scope nonce)
function setThemeCookie(themeName) {
    const d = new Date();
    d.setTime(d.getTime() + (30 * 24 * 60 * 60 * 1000));
    let expires = "expires=" + d.toUTCString();
    // Ditambahkan flag Secure; SameSite agar kompatibel dengan keamanan browser
    document.cookie = "user_theme=" + themeName + ";" + expires + ";path=/;Secure;SameSite=Strict";
    window.location.reload();
}

$(document).ready(function() {
    // 1. Event Listener untuk Tombol Tema (Theme Selector)
    $(document).on('click', '.theme-selector', function() {
        var selectedTheme = $(this).data('theme');
        setThemeCookie(selectedTheme);
    });

    // 2. Event Listener untuk Modal Legal (Privacy, Terms, Help, Tagline)
    $('.btn-legal-trigger').on('click', function() {
    ...
    // 4. Logika UI Sidebar & Palette
    const $btnHamburger = $('#btn-hamburger');
    const $sidebarMobile = $('#sidebar-mobile');
    const $iconBars = $('#icon-bars');
    const $iconX = $('#icon-x');

    $('#main-palette-btn').on('click', function(e) {
        e.stopPropagation();
        $('#palette-menu').toggleClass('active');
    });

    $(document).on('click', function() {
        $('#palette-menu').removeClass('active');
    });

    $('#palette-menu').on('click', function(e) {
        e.stopPropagation();
    });
    ...
    // 5. Logika Modal Content & Functions
